...
namespace KognaServer.Server.KinematicEngine
{
    ...
    public struct MCodeAction
    {
        ...
        {
            Action = action;
            DParams = new double[RS274NGC.MAX_MCODE_DOUBLE_PARAMS];
    ...
    {
        public static string Translate(string input) => RS274NGC.Translate(input);
    }
    ...
    public class GCodeInterpreter : IDisposable
    {
        ...
        public GCodeInterpreter(CCoordMotion coordMotion)
        {
            ...
            McodeActions = new MCodeAction[RS274NGC.MAX_MCODE_ACTIONS];
        ...
        /// </summary>
        public int Interpret(string fileName, int start, int end, int restart,
                            GStatusCallback statusFn, GCompleteCallback completeFn)
        {
        ...
        private int DoExecute()
        {
            ...
            int status = RS274NGC.Init();
            if (status != RS274NGC.OK) return ExitWithError(status);

            if (!string.IsNullOrEmpty(SetupFile) && (status = RS274NGC.ReadSetup(SetupFile)) != RS274NGC.OK)
                return ExitWithError(status);
            if (!string.IsNullOrEmpty(VarsFile) && (status = RS274NGC.ReadVars(VarsFile)) != RS274NGC.OK)
                return ExitWithError(status);
            if (!string.IsNullOrEmpty(GeoFile) && (status = RS274NGC.ReadGeo(GeoFile)) != RS274NGC.OK)
                return ExitWithError(status);
            if (!string.IsNullOrEmpty(ToolFile) && (status = RS274NGC.ReadTool(ToolFile)) != RS274NGC.OK)
                return ExitWithError(status);

            int programStatus = RS274NGC.OK;
            using (var reader = new StreamReader(m_InFile))
            {
                string line;
                while ((programStatus == RS274NGC.OK) && reader.Peek() >= 0)
                {
                    if (m_Halt) break;
                    ...
                    programStatus = RS274NGC.Execute(line);
                    if (programStatus != RS274NGC.OK)
            ...
            SetupTracker.InsertState(ref _setup);
        ...
        public bool GetAbort() => CoordMotion.AbortFlag;

        public int InitializeInterp() => RS274NGC.Init();
        public int ExecutePC(string name, bool noWait = false) => RS274NGC.ExecutePC(name, noWait);
        public int DoResumeSafe() => RS274NGC.ResumeSafe();
        ...
        // --- Motion parameters ---
        public MotionParams GetMotionParams() => CoordMotion.Kinematics.MotionParams;
        ...
        public static void OnRapidSegment(SEGMENT seg)
        {
            ...
            Canon.STRAIGHT_TRAVERSE(
                ...
                seg.b,
                seg.c,
                noCallback: true,   // or false if you still want callbacks
                seq: seg.sequence,  // or however you track it
                id: seg.id
            );
        }
    }
